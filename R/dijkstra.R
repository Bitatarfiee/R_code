...
#' @details Should assert that the graph argument have the above structure and that init_node is a numeric scalar that exist in the graph.
#' @export dijkstra
#' @keywords wiki_graph
#' @examples 
#' wiki_graph <-
...
dijkstra <- function(graph,init_node){
  ...
  node_matrix <- matrix(ncol=2, c(graph$v1,graph$v2))
  graph_uniqe <- graph[!duplicated(graph["v1"]), ]
  g_edges <- add_edges(make_empty_graph(nrow(graph_uniqe)), t(node_matrix), weight=graph$w)
  distance <- distances(g_edges, mode="out")
  return(distance[,init_node])
}
